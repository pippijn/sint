...
pub fn print_trajectory(initial_state: GameState, path: Vec<(PlayerId, Action)>) {
    ...
    println!("Start: Hull {}, Boss {}, Players {}", state.hull_integrity, state.enemy.name, state.players.len());
    ...
    for (pid, action) in path {
        ...
        let prev_hull = state.hull_integrity;
        let prev_hazards = count_hazards(&state);
            ...
            Ok(new_state) => {
                ...
                if state.phase != prev_phase {
                     if state.turn_count > current_round {
                         print_results(prev_hull, state.hull_integrity, prev_hazards, count_hazards(&state));
                         current_round = state.turn_count;
                         println!("\n--- ROUND {} ---", current_round);
                         // Don't print header here, wait for phases
                     }
                     
                     if state.phase == GamePhase::MorningReport && prev_phase != GamePhase::MorningReport {
                         ...
                         if !state.active_situations.is_empty() {
                            let names: Vec<String> = state.active_situations.iter().map(|c| c.title.clone()).collect();
                            println!("[ACTIVE] {}", names.join(", "));
                         }
                     } else if state.phase == GamePhase::TacticalPlanning && prev_phase != GamePhase::TacticalPlanning {
...
fn print_planning_context(state: &GameState) {
    ...
    if let Some(attack) = &state.enemy.next_attack {
        println!("[ENEMY] {} targets Room {} with {:?}", state.enemy.name, attack.target_room, attack.effect);
    }
    ...
    for rid in room_ids {
        if let Some(room) = state.map.rooms.get(&rid) {
            let fires = room.hazards.iter().filter(|&h| *h == HazardType::Fire).count();
            let waters = room.hazards.iter().filter(|&h| *h == HazardType::Water).count();
            
            if fires > 0 { fire_rooms.push(format!("{} (x{})", rid, fires)); }
            if waters > 0 { water_rooms.push(format!("{} (x{})", rid, waters)); }
        }
    }
    ...
    // 5. Status
    println!("[STATUS] Hull: {} | Boss HP: {} | Total Hazards: {}", state.hull_integrity, state.enemy.hp, count_hazards(state));
