...
pub fn generate_tasks(state: &GameState, player_id: &str) -> Vec<PlayerTask> {
        ...
        if room.items.contains(&ItemType::Peppernut) {
            if ap >= 1 {
                 tasks.push(PlayerTask {
                    ...
                    actions: vec![Action::PickUp { item_type: ItemType::Peppernut }],
                    cost: 1,
                 });
            }
        }
    ...
    if let Some(_room) = state.map.rooms.get(&start_room) {
        ...
        for pid in &player_ids {
            ...
            if other.room_id == start_room && other.status.contains(&sint_core::types::PlayerStatus::Fainted) {
                if ap >= 1 {
                    tasks.push(PlayerTask {
                        player_id: player_id.to_string(),
                        description: format!("Revive {}", other.name),
                        actions: vec![Action::Revive { target_player: other.id.clone() }],
                        cost: 1,
                    });
                }
            }
