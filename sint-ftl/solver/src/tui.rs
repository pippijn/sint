...
pub fn run(status: Arc<Mutex<SearchStatus>>) {
    ...
    let layout = LinearLayout::horizontal()
        .child(Panel::new(stats_view).title("Search Status").fixed_width(40))
        .child(Panel::new(ScrollView::new(path_view)).title("Best Path So Far").full_width());
