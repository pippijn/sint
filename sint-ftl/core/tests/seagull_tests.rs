use sint_core::{Action, GameLogic, GamePhase, ItemType, CardId};
...
fn test_seagull_attack_validation_in_planning() {
    ...
    let seagull_card = state.deck.iter().find(|c| c.id == CardId::SeagullAttack).cloned().unwrap();
    ...
    assert!(res.is_err(), "Should be blocked by Seagull Attack validation");
    if let Err(e) = res {
        assert!(e.to_string().contains("Seagull Attack"), "Error should mention Seagull Attack");
    }
}
...
fn test_seagull_attack_refund_in_execution() {
    ...
    let state = GameLogic::apply_action(state, "P1", Action::PickUp { item_type: ItemType::Peppernut }, None).unwrap();
    ...
    let mut state = GameLogic::apply_action(state, "P1", Action::Move { to_room: 7 }, None).unwrap();
    ...
    // Refund: +1. Final: 1.
    assert_eq!(state.players["P1"].ap, 1, "Should have refunded 1 AP for failed move");
}
