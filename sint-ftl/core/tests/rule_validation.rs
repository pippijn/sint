...
fn test_repair_priority_hierarchy() {
    ...
    assert!(!state.map.rooms[&cargo_id].hazards.contains(&HazardType::Water), "First repair should remove water");
    assert_eq!(state.map.rooms[&cargo_id].system_health, 1, "System health should not have changed yet");
    ...
    assert_eq!(state.map.rooms[&cargo_id].system_health, 2, "Second repair should restore system HP");
    assert_eq!(state.hull_integrity, 10, "Hull should still not have changed");

    // --- REPAIR 3: Should restore SYSTEM HP to full ---
    state = GameLogic::apply_action(state, "P1", Action::Game(GameAction::Repair), None).unwrap();
    resolution::resolve_proposal_queue(&mut state, false);
    
    assert_eq!(state.map.rooms[&cargo_id].system_health, 3, "Third repair should restore system HP to full");
    assert!(!state.map.rooms[&cargo_id].is_broken, "System should no longer be broken");
    assert_eq!(state.hull_integrity, 10, "Hull should still not have changed");
    ...
    assert_eq!(state.hull_integrity, 11, "Fourth repair should finally restore Hull integrity in Cargo");
}
...
fn test_system_roll_to_room_mapping() {
    ...
    assert_eq!(attack_roll_2.target_room, 1, "Roll 2 (Bow) should target Room 1 in Star layout");

    let attack_roll_3 = simulate_roll(3);
    assert_eq!(attack_roll_3.target_system, Some(SystemType::Dormitory));
    assert_eq!(attack_roll_3.target_room, 2, "Roll 3 (Dormitory) should target Room 2 in Star layout");

    let attack_roll_10 = simulate_roll(10);
    assert_eq!(attack_roll_10.target_system, Some(SystemType::Storage));
    assert_eq!(attack_roll_10.target_room, 9, "Roll 10 (Storage) should target Room 9 in Star layout");
}
