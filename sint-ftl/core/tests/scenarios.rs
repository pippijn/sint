use sint_core::{GameLogic, Action, GamePhase, HazardType, ItemType, CardId};
...
fn test_scenario_fire_drill() {
    ...
    if let Some(p) = state.players.get_mut("P1") { p.room_id = 3; p.ap = 2; }
    if let Some(p) = state.players.get_mut("P2") { p.room_id = 7; p.ap = 2; }

    // Hazards: 2 Fire in Kitchen (6), 1 Fire in Cargo (4)
    if let Some(r) = state.map.rooms.get_mut(&6) { r.hazards = vec![HazardType::Fire, HazardType::Fire]; }
    if let Some(r) = state.map.rooms.get_mut(&4) { r.hazards = vec![HazardType::Fire]; }
    ...
    fn advance_to_planning(mut state: sint_core::GameState) -> sint_core::GameState {
        ...
        while state.phase != GamePhase::TacticalPlanning && state.phase != GamePhase::GameOver {
            ...
            for pid in pids {
                ...
                if let Ok(new_state) = GameLogic::apply_action(state.clone(), &pid, Action::VoteReady { ready: true }, None) {
                    state = new_state;
                }
            }
            attempts += 1;
            if attempts > 20 { panic!("Stuck in loop phase: {:?}", state.phase); }
        }
        state
    }
...
fn test_scenario_bucket_brigade() {
    let mut state = GameLogic::new_game(vec!["P1".to_string(), "P2".to_string(), "P3".to_string()], 12345);
    state.phase = GamePhase::TacticalPlanning;

    // Setup
    // P1 in Kitchen(6), has Nut.
    if let Some(p) = state.players.get_mut("P1") { p.room_id = 6; p.inventory = vec![ItemType::Peppernut]; p.ap = 2; }
    // P2 in Hallway(7).
    if let Some(p) = state.players.get_mut("P2") { p.room_id = 7; p.inventory = vec![]; p.ap = 2; }
    // P3 in Cannons(8).
    if let Some(p) = state.players.get_mut("P3") { p.room_id = 8; p.inventory = vec![]; p.ap = 2; }
    
    // P1 Throws to P2
    state = GameLogic::apply_action(state, "P1", Action::Throw { target_player: "P2".to_string(), item_index: 0 }, None).unwrap();
    
    // P2 Throws to P3 (Dependent on P1's action)
    state = GameLogic::apply_action(state, "P2", Action::Throw { target_player: "P3".to_string(), item_index: 0 }, None).unwrap();
...
fn test_scenario_kraken_fight() {
    let mut state = GameLogic::new_game(vec!["P1".to_string(), "P2".to_string(), "P3".to_string(), "P4".to_string()], 12345);
    state.phase = GamePhase::TacticalPlanning;
    ...
    // Fog Bank Active
    let fog = state.deck.iter().find(|c| c.id == CardId::FogBank).cloned().unwrap();
    ...
    // P1 Shoot Kraken.
    if let Some(p) = state.players.get_mut("P1") { p.room_id = 8; p.inventory = vec![ItemType::Peppernut]; p.ap = 2; }
